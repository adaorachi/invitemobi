class Event < ApplicationRecord
  ...
  scope :invited_past, ->(current_user) { EventAttendee.joins(:attended_event).where('attendee_id = ? AND status == ? AND start_date <= ?', current_user, 'attending', Time.now.midnight) }
  scope :invited_upcoming, ->(current_user) { EventAttendee.joins(:attended_event).where('attendee_id = ? AND status == ? AND start_date > ?', current_user, 'attending', Time.now.midnight) }
